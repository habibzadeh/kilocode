...
import { effectCmd } from "../effect-cmd" // kilocode_change
    ...
    yield* Effect.promise(async () => {
      ...
      if (args.file) {
        ...
        for (const filePath of list) {
          ...
          files.push({
            type: "file",
            url: pathToFileURL(resolvedPath).href, // kilocode_change
      ...
      async function execute(sdk: KiloClient) {
        ...
        if (KiloRun.isBuiltin(args.command)) {
          await KiloRun.runBuiltin(sdk, sessionID, args.command, args.model, directory)
          return
        }
        // kilocode_change end
